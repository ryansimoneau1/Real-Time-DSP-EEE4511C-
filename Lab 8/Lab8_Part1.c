/*
 ...
 Description: This program implements a 512 point DFT utilizing pingpong registers and outputs the maximum magnitude in dB along with
    ...
    while(1)
    {
        if(DFTFlag == 0 && BufferFlag == 0)
        {
            ...
            for(Uint16 k = 0; k < 256; k++)
            {
                ...
                Xksquare = pow(Xkreal + Xkimaginary, 2);
