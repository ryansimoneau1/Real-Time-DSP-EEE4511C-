...
int16           TestBuff  [FFT_SIZE];
float32         FFTin     [2*FFT_SIZE];
float32         FFTout    [2*FFT_SIZE];
...
#pragma DATA_SECTION(TestBuff,   "ramgs7")
       ...
       if(FFTflag)
        {
           ...
           while(inccnt < FFT_SIZE){ // while loop to fill FFTin with data from InCir
               ...
               FFTin[fftcnt] = Hann*((float32)(Input[inccnt]));
               fftcnt = fftcnt + 2;
               inccnt++;
           }
           fftcnt = 0;
...
//Uint16 testcnt   = 0;
//Uint16 testcnt2  = 0;
//while(testcnt < FFT_SIZE){
//    TestBuff[testcnt] = (int16)(FFTin[testcnt2]);
//    testcnt2 = testcnt2 + 2;
//    testcnt++;
//}
//testcnt   = 0;
...
//// Extract the real data from the IFFT
//Uint16 ifftcnt = 0;
//Uint16 outcnt  = 0;
//while(outcnt < FFT_SIZE){ // while loop to fill FFTin with data from InCir
//    Output[outcnt] = (int16)(FFTout[ifftcnt]);
//    ifftcnt = ifftcnt + 2;
//    outcnt++;
//}
//ifftcnt = 0;
