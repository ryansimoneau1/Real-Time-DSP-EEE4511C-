...
interrupt void DIP1_isr(void){ // DIP 1 (32 in, 32 out)
    ...
    command = CLKsampleratecontrol (SR32); // change this value //
    SpiTransmit(command);
    SmallDelay();
...
interrupt void DIP2_isr(void){ // Switch 2 (48 in, 8 out)
    ...
    // change the sample frequency
    command = CLKsampleratecontrol (SR48to8); // change this value //
    SpiTransmit(command);
    SmallDelay();
...
// DECIMATION FUNCTION //
//if(DecimationFlag == 1){
    //for(Uint16 DecimationPointer = 0; DecimationPointer < 0x80000; DecimationPointer = DecimationPointer +2)
    //{
       ...
       // Sp = SRAMREAD(DecimationPointer); // S0
       // DecimationPointer = DecimationPointer +2;
       // SRAMWRITE(Sp,DecimationPointer); // S01
       // DecimationPointer = DecimationPointer +2;
        //SRAMWRITE(Sp,DecimationPointer); // S02
        //DecimationPointer = DecimationPointer +2;
       // SRAMWRITE(Sp,DecimationPointer); // S03
        //DecimationPointer = DecimationPointer +2;
...
//GpioDataRegs.GPASET.all |= (0xFF << 4); // indicates when the for loop has been exited
