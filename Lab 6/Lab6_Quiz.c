...
float Fw = 500; // frequency of the wave in Hz
float Fs = 48000; // frequncy of sampling in Hz
//Uint32 FuturePointer = 0; // pointer used to grab past data from SRAMs
int16 Sf = 0; // sample from the future
...
int main(void){
    ...
    Uint16 period = ((Uint16)(Fs/Fw));
    Uint16 DelaySamples = ((Uint16)((Fs/Fw)*0.25));
    ...
    while(1){ // top level while loop to cycle between which mode we're in
            ...
            while(PlayKey == 1){
                ...
                Sf = SRAMREAD(PlayPointer + period + DelaySamples);
                Play = Sf;
