/*
 ...
 Description: This program creates an echo effect upon output
...
// Reverb variables
Uint16 n = 0; // multiples of the delay time
#define p  480; // delay time in units of samples
float a = 0.5; // gain of the reverb effect
Uint16 DelayTime = 0; // delay time p*n
Uint32 PastPointer = 0; // pointer used to grab past data from SRAMs
int16 Sp = 0; // sample from the past
int16 Sc = 0; // sample from the present
...
int main(void){
    ...
    while(1){ // top level while loop to cycle between which mode we're in
            ...
            n = (~(GpioDataRegs.GPADAT.all) & 0xF);
            DelayTime = n*p; // delay time in samples
            ...
            while(PlayKey == 1){
                ...
                if(PlayPointer < DelayTime)
                {

                    PastPointer = 0x80000 + PlayPointer - DelayTime;

                }else{

                    PastPointer = PlayPointer - DelayTime;

                }


                Sc = SRAMREAD(PlayPointer);
                Sp = SRAMREAD(PastPointer);
                Play = (int16)((1 - a)*((float)Sc) + a*((float)Sp));
                SRAMWRITE(Play,PlayPointer);
