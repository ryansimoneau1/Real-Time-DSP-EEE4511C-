/*
 Lab6_Part3_Interpolation.c
 Ryan Simoneau
 Last Modified: Mar, 07 2021

 Description: This program records and plays back audio at different frequencies while also interpolating
...
Uint16 volatile DIP1test = 0;
Uint16 volatile DIP2test = 0;
    ...
    while(1){ // top level while loop to cycle between which mode we're in
            ...
            while(RecordKey == 1){
                    ...
                    if(InterpolationFlag == 0)
                    {
                        ...
                        while(DataSend == 0);
                        Sp = Record; // load value into the current // there could be an issue with justification. check to see where valid data is in Record //
                        InterpolationFlag = 1;
                        DataSend = 0;
                    ...
                    }else
                    {
                        ...
                        while(DataSend == 0);
                        Sc = Record; // load value into the previous // there could be an issue with justification. check to see where valid data is in Record //
                        delta = (Uint16)(((float)Sc)*0.25 + ((float)Sp)*0.25);

                        Interp1 = Sp + delta;
                        Interp2 = Sp + 2*delta;
                        Interp3 = Sp + 3*delta;

                        SRAMWRITE(Sp,RecordPointer);
                        RecordPointer = RecordPointer + 2;
                        SRAMWRITE(Interp1,RecordPointer);
                        RecordPointer = RecordPointer + 2;
                        SRAMWRITE(Interp2,RecordPointer);
                        RecordPointer = RecordPointer + 2;
                        SRAMWRITE(Interp3,RecordPointer);
                        RecordPointer = RecordPointer + 2;
                        SRAMWRITE(Sc,RecordPointer);
                        RecordPointer = RecordPointer + 2;

                        Sp = Sc; // current value becomes the previous
                        DataSend = 0;
...
interrupt void PB3_isr(void){ // Button 3 (Play)
    ...
    XintRegs.XINT5CR.bit.ENABLE = 0; // disable interrupts to avoid bouncing
    DELAY_US(5E3);
    XintRegs.XINT5CR.bit.ENABLE = 1; // re-enable after bouncing is gone
...
interrupt void DIP1_isr(void){ // DIP 1 (8 in, 8 out)
    ...
    command = CLKsampleratecontrol (SR8); // change this value //
    SpiTransmit(command);
    SmallDelay();

    PieCtrlRegs.PIEACK.all = PIEACK_GROUP12; // CHECK THIS IN THE MORNING //

}

interrupt void DIP2_isr(void){ // Switch 2 (8 in, 48 out)

    DIP2test++;

    XintRegs.XINT4CR.bit.ENABLE = 0; // disable interrupts to avoid bouncing
    DELAY_US(5E3);
    XintRegs.XINT4CR.bit.ENABLE = 1; // re-enable after bouncing is gone

    // change the sample frequency
    command = CLKsampleratecontrol (SR8to48); // change this value //
    SpiTransmit(command);
    SmallDelay();
...
void GPIOintEnb(void){
    ...
    GPIO_SetupXINT3Gpio(0);
    GPIO_SetupXINT4Gpio(1);
    EALLOW;

    PieCtrlRegs.PIEIER1.bit.INTx4 = 1; // XINT1 PB1
    PieCtrlRegs.PIEIER1.bit.INTx5 = 1; // XINT2 PB2
    PieCtrlRegs.PIEIER12.bit.INTx1 = 1; // XINT3 DIP1
    PieCtrlRegs.PIEIER12.bit.INTx2 = 1; // XINT4 DIP2
    ...
    XintRegs.XINT3CR.bit.ENABLE = 1; // DIP1
    XintRegs.XINT4CR.bit.ENABLE = 1; // DIP2
    XintRegs.XINT5CR.bit.ENABLE = 1; // PB3


    PieVectTable.XINT1_INT = &PB1_isr; // Put push button 1 here //
    PieVectTable.XINT2_INT = &PB2_isr;
    PieVectTable.XINT3_INT = &DIP1_isr;
    PieVectTable.XINT4_INT = &DIP2_isr;
