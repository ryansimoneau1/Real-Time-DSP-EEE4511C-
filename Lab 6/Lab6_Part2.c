/*
 ...
 Description: This program modifies the sample frequency of the codec
...
interrupt void DIP1_isr(void){ // Switch 1 (8kHz)
...
interrupt void DIP2_isr(void){ // Switch 2 (32kHz)
...
interrupt void DIP3_isr(void){ // Switch 3 (48kHz)
...
interrupt void Mcbspb_isr(void) {
    ...
    test++;
    // toggle GPIO pin 22 upon entering the interrupt
...
void DIPintEnb(void){
    ...
    GPIO_SetupXINT3Gpio(2);
    EALLOW;

    PieCtrlRegs.PIEIER1.bit.INTx4 = 1; // XINT1 DIP1
    PieCtrlRegs.PIEIER1.bit.INTx5 = 1; // XINT2 DIP2
    PieCtrlRegs.PIEIER12.bit.INTx1 = 1; // XINT3 DIP3
    ...
    PieVectTable.XINT1_INT = &DIP1_isr;
    PieVectTable.XINT2_INT = &DIP2_isr;
    PieVectTable.XINT3_INT = &DIP3_isr;
