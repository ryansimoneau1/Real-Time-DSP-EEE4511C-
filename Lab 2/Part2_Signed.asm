...
N			.set	7 ; arbitrary values to avoid hard coding (# of #s in vector)
A			.set	15 ; the value being compared against
...
vector					 ; a vector of arbitrary values
			.byte	5
			.byte	-14
			.byte	-12
			.byte	-16
...
NEWV	; return here once the previous value is compared to load a new value
		...
		; check if A is less than or equal to what's in AL (signed)
		SUB		AH,AL
		LSL		ACC,8 ; push lower 8 bits to upper 8 bits
		B		SUMX,LEQ ; branch to SUMX if A <= TableVal
